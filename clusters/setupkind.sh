...
function printHelp() {
  echo "Usage: "
  echo "    $0 --cluster-name dev-cluster --k8s-release 1.22.1 --ip-octet 255"
  echo ""
  echo "Where:"
...
PREFIX=$(docker network inspect -f '{{range .IPAM.Config }}{{ .Gateway }}{{end}}' kind | cut -d '.' -f1,2,3)
...
while : ; do
  ...
  if [[ -n "${IP}" ]]; then
    ...
    kubectl config set clusters.kind-"${CLUSTERNAME}".server https://"192.168.0.103":6443
    break
  fi
